...
data "google_compute_subnetwork" "pub_subnet" {
  name   = "${var.subnet}"
  region = "${var.region}"
}
...
resource "google_compute_firewall" "ranger" {
  ...
  source_ranges = ["${var.local_ips}","${data.google_compute_subnetwork.pub_subnet.ip_cidr_range}","${data.google_compute_subnetwork.pub_subnet.gateway_address}"]
...
output "public_subntetwork_cidr" {
  value = "${data.google_compute_subnetwork.pub_subnet.ip_cidr_range}"
}
